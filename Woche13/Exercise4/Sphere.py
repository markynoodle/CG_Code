...
class Sphere:
    def __init__(self, rotational_speed, rotational_axis, move_direction, color):
        self.translation_matrix = glm.mat4()
    ...
    def translate(self, translation_vector):
        self.translation_matrix = glm.translate(self.translation_matrix, translation_vector)

    def rotate(self):
        self.rotation_matrix = glm.rotate(self.rotation_matrix, math.pi / 180 * self.rotational_speed, self.rotational_axis)
    ...
    def draw(self, view_matrix, projection_matrix):
        ...
        # move
        self.translation_matrix = glm.translate(self.translation_matrix, self.move_direction/10000)

        self.model_matrix = self.translation_matrix*self.rotation_matrix
        ...
        color_loc = glGetUniformLocation(self.shader_program_sphere, 'color')
    ...
    def compute_vertices(self, stack_count, sector_count):
        ...
        for i in range(stack_count + 1):
            ...
            xy = self.radius * math.cos(stackAngle)  # r * cos(u)
            z = self.radius * math.sin(stackAngle)  # r * sin(u)
    ...
    def check_collision(sphere1, sphere2):
        return glm.length(sphere1.translation_matrix[3].xyz-sphere2.translation_matrix[3].xyz) <= (sphere1.radius + sphere2.radius) # if there is a scaling transformation, then it has to be taken into account here.