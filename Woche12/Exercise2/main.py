...
model_matrix_2 = glm.mat4() # creates a 4x4 identity matrix
...
def key_callback(window, key, scancode, action, mods):
    global view_matrix, camera_position, model_matrix
...
while (glfw.get_key(window, glfw.KEY_ESCAPE) != glfw.PRESS and not glfw.window_should_close(window)):
    ...
    # rotate the other model as you wish
    model_matrix_2 = glm.rotate(model_matrix_2, 0.01, glm.vec3(1, 0, 1))
    ...
    glUniformMatrix4fv(model_loc, 1, GL_FALSE, glm.value_ptr(model_matrix_2))
    glDrawElements(GL_TRIANGLES, len(indices), GL_UNSIGNED_INT, None)
